...
<<<<<<< HEAD:Documentation/Documentation DSF Project.A.tex
\usepackage{hyperref} % Hyperlinks
\usepackage{subfig} % Package fï¿½r mehrere Grafiken nebeneinander
=======

\usepackage{subfig} % Package für mehrere Grafiken nebeneinander
>>>>>>> 73443b9fb830616b744a442c2fbfed6171cf4585:Documentation/Documentation DSF Project.tex
\usepackage{amsmath}
...
To amend the New York City inspection data with location information of subway stations, we made use of \href{https://data.ny.gov/Transportation/NYC-Transit-Subway-Entrance-And-Exit-Data/i9wp-a4ja}{this official dataset provided by the State of New York}, which contains the location of every subway station and applied the Haversine Formula to find the distance of the closest station to every shop.

\subsection{Demographic Data}

An ample amount of demographic data on the USA and New York City exists freely available on the internet. The quality of the available datasets differs strongly, however. We judged the data provided by the U.S. census bureau to be the most reliable, mainly due to the large sample sizes, that their estimates were based on. Based on their data, two datasets, one conveying demographic information by U.S.-county and one by Census Tract, were created (US Census Bureau, 2017), which can also be found on href\{https://www.kaggle.com/muonneutrino/us-census-demographic-data}{Kaggle}.\\
\\
Whilst the first dataset could be directly merged via the county-variable, direct merging with the second dataset was not possible, since Census Tracts do not follow the traditional pattern of location specification used in the inspection?s dataset. An approach of nevertheless matching the sets, was found in the creation of a third ?translation? dataset AddTrac (Address to Census Tract) which matched the retailers? locations with the Census Tract codes. To do so, we made use of the census bureau?s geocoding service (US Census Bureau, n.d., Geocoder).\\
\\
This service added various geolocation-identifiers to specifically created csv files holding our food retailers' addresses. Unfortunately, circa 20\% of the addresses were not identified by the service (possible reasons include name changes and confidentiality concerns (U.S. Census Bureau, 2018, p. 7)) which led to some data loss. The address specific Census Tract Ids were ultimately created by combining the output of state FIPS , county FIPS and census block codes, as well as adding placeholder zeros where necessary.
...
As discussed above and visible in figure \ref{ice} even the most important variables do not have a significant impact on the predictions. Thus, additional and more relevant parameters are needed in order to achieve a better performance.
...
Despite the utilization of an array of different variables and methods, the prediction of inspection grades proved to be exceedingly difficult. We believe that the causes for a certain rating can be found in the characteristics of the restaurant themself and not certain attributes of the neighborhood like we use. Therefore, things like the age of the owner, his education or what kind of products are offered would have been more valuable predictors. In addition, the imbalance in the classes further complicated the estimation of accurately predicting models. The over- and under-bagging was computationally too intensive and did not lead to the results we expected. In a similar situation we would, therefore, try to overcome the imbalance issue with other means like the more advanced Synthetic Minority Oversampling TEchnique (SMOTE) that artificially creates new observations of the minority data.
